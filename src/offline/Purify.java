...
public class Purify {
	public static void main(String[] args) {
		...
		MongoDatabase db = mongoClient.getDatabase(MongoDBUtil.DB_NAME);
                             // “/User/xxx/Downloads”
		String fileName = "/Users/bhl/Documents/laiproject/tomcat_log.txt";

		try {
			db.getCollection("logs").drop();  //drop if exists
			
			FileReader fileReader = new FileReader(fileName);
			BufferedReader bufferedReader = new BufferedReader(fileReader);  // speed up reading;
			String line = null;
			while ((line = bufferedReader.readLine()) != null) {
				...
				Pattern pattern = Pattern.compile("\\[(.+?):(.+)");  // match 19/Aug/2017:22:00:24
